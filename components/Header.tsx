  ...
  return (
    ...
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-sm border-b border-zinc-100 py-6 px-6 md:px-12 flex justify-between items-center transition-all h-24">
      ...
      <div className={`flex items-center gap-6 md:gap-8 ${isMobileSearchOpen ? 'w-full md:w-auto' : ''}`}>
        {/* Search Bar */}
        <div className={`relative flex items-center transition-all ${isMobileSearchOpen ? 'w-full border-b border-zinc-800 pb-1' : 'md:border-b md:border-zinc-200 md:pb-1 md:focus-within:border-zinc-800'}`}>
            {isMobileSearchOpen ? (
               <Search size={16} className="text-ink shrink-0" />
            ) : (
               <div className="relative group flex items-center">
                 <button onClick={() => setIsMobileSearchOpen(true)} className="md:pointer-events-none">
                   <Search size={16} className="text-zinc-400 md:text-zinc-400" />
                 </button>
                 {/* Tooltip for Search */}
                 <div className="absolute left-1/2 -translate-x-1/2 top-full mt-4 hidden md:group-hover:block z-50 pointer-events-none">
            ...
            <input 
              ...
              className={`bg-transparent outline-none text-sm font-sans ml-3 text-ink placeholder-zinc-300 ${isMobileSearchOpen ? 'block w-full' : 'hidden md:block w-48'}`}
              autoFocus={isMobileSearchOpen}
            />

            {/* Loading Indicator for Search */}
            {(isSearching && searchQuery.length > 0) && (
              <Loader2 size={14} className="animate-spin text-zinc-400 mx-2 shrink-0" />
            )}

            {isMobileSearchOpen && (
              <button onClick={() => { setIsMobileSearchOpen(false); setSearchQuery(''); }} className="ml-2 text-zinc-400">
                <X size={16} />
              </button>
            )}
        </div>
